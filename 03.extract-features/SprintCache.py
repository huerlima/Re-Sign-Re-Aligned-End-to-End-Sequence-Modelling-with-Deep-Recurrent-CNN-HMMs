...
class FileArchive:
  ...
  def _raw_read(self, size, typ):
    if typ == "str":
      ...
      flag = self.read_u32()  # ?
      typ = self.read_str(8)
      if typ == "ALIGNRLE" or typ == "AALPHRLE":
        size = self.read_U32()
        if size < (1 << 31):
          # RLE scheme
          time = 0
  ...
  def showAllophone(self, state):
        #    for a in self.archives.values():
    return self.allophones[state]
...
class FileArchiveBundle():
  ...
  def showAllophone(self, state):
#    for a in self.archives.values():
    return self.archives.values()[0].allophones[state]
...
class MixtureSet:
  ...
  def setWeights(self, weightsList):
    ...
    self.nMeans = n
    self.means = numpy.zeros([self.nMeans, self.dim], dtype=numpy.float64)
    self.meanWeights = numpy.array(weightsList)
    self.nCovs = n
    self.covs = numpy.zeros([self.nCovs, self.dim], dtype=numpy.float64)
    self.covWeights = numpy.zeros(self.nCovs, dtype=numpy.float64)
    self.nDensities = n
    ...
    for n in range(self.nMixtures):
      ...
      for i in range(nDensities):
        dns_idx.append(i)
        dns_weight.append(weightsList[n])
      self.mixtures[n] = (dns_idx, dns_weight)
